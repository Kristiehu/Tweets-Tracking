...
# data = {'tweet': [], 'user': [], 'Coord_x': [], 'Coord_y': [], 'date':[], 'time': []}
...
class CustomStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, status):
        ...
        try:
            ...
            # data['coord_x'].append(coord_x)
            # data['coord_y'].append(coord_y)
            # data['date'].append(date_est)
            # data['time'].append(h_m_s_est)
...
# Change the values to track what you have chosen to map (the keywords to search)
listener.filter(track=['#travel', '#travelling'])

# *****Save your dataframe to a csv file*****
s = pd.DataFrame(df)
s.to_csv('C:/geog 381/a03_tweet/data.csv', encoding="utf-8")
